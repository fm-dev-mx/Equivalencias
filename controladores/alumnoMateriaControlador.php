<?php
    ...
    class alumnoMateriaControlador extends alumnoMateriaModelo{
		public function validar_materias_controlador($materias){						
			...
			var_dump($codigoAlumno);
			$consulta=mainModel::ejecutar_consulta_simple("SELECT CodigoMateria FROM alumnomaterias WHERE CodigoAlumno='$codigoAlumno'");
			$ban=0;
			...
			if($consulta->rowCount()>=1){
				...
				if($ban==0){
					...
					$alerta=[
						...
						"Texto"=>"No hemos podido registrar las materias correctamente, favor de intentar nuevamente!",
						"Tipo"=>"error"
					];
				}	
			}
